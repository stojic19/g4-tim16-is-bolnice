...
namespace Bolnica.Izvestaj.Lekar
{
    public class ReceptiIzvestaj
    {
        ...
        public void ExportDataTableToPdf(DataTable dtblTable, String strPdfPath, string strHeader)
        {
            ...
            document.Add(new Paragraph(new Chunk(new iTextSharp.text.pdf.draw.LineSeparator(0.0F, 100.0F, Color.BLACK, Element.ALIGN_LEFT, 1))));
            document.Add(new Chunk("\n"));

            document.Add(KreiranjeTabele(dtblTable));
            document.Close();
            writer.Close();
            fs.Close();
        }
        ...
        public DataTable MakeDataTable(List<ReceptDTO> recepti)
        {
            ...
            foreach (ReceptDTO r in recepti)
            {
